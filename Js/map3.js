...
function setAllMap(valor) {
    for (var m in markers) {
        markers[m].setMap(valor);
    }
}


function carga_punto_ReporteAP(lat, lon, movil, fe) {

    var icono = "./images/Mavigps-Iconos/Movil/Movil_Sentido/NE2.png";
    ...
    var label = new Label();
    label.bindTo('map', markers[1]);
    label.bindTo('position', markers[1], 'position');
    label.bindTo('text', markers[1], 'title');

    google.maps.event.addListener(markers[1], 'click', function () {
        if (!popup) {
            popup = new google.maps.InfoWindow();
        }
        var note = '<a style=" font-size:small; color:Black; font-family:Calibri">Patente: ' + movil + " <br />"
                                + 'Fecha: ' + fe + " <br />";
        note = note + "</a>";
...
function marcaTracto(name, lat, lon, fec, dir, vel, id, transportista) {
    var icono
    if (dir > 338 || dir <= 22)
        icono = "../images/tractos/tracto_N.png";

    else if (dir > 22 && dir <= 67)
        icono = "../images/tractos/tracto_NE.png";

    else if (dir > 67 && dir <= 112)
        icono = "../images/tractos/tracto_E.png";

    else if (dir > 112 && dir <= 157)
        icono = "../images/tractos/tracto_SE.png";

    else if (dir > 157 && dir <= 202)
        icono = "../images/tractos/tracto_S.png";

    else if (dir > 202 && dir <= 257)
        icono = "../images/tractos/tracto_SO.png";

    else if (dir > 257 && dir <= 292)
        icono = "../images/tractos/tracto_O.png";

    else if (dir > 292 && dir <= 338)
        icono = "../images/tractos/tracto_NO.png";

    else
        icono = "../images/tractos/tracto_N.gif";

    marker(id, name, icono, new google.maps.LatLng(lat, lon), fec, vel, transportista);
}
...
function BorraMovil() {
    window.parent.RadWindow2.document.getElementById('txt_movsel').innerHTML = '';
    window.parent.RadWindow2.desselecciona();
}


function marker(id, marca, icono, location, fec, vel, transportista) {
    if (markers[id]) {
...
function marca_movil_viaje(name, lat, lon, fec, dir, vel) {
    ...
    marker_movil_viaje(1, name, icono, new google.maps.LatLng(lat, lon), fec, vel, vel);
}

function marker_movil_viaje(id, marca, icono, location, fec) {
    if (markers[id]) {
        markers[id].setPosition(location);
        markers[id].setIcon(icono);
    }
...
function procesa() {
        ...
        if (nombre != arreglo[0]) {
            if (is_poligono == "0") {
                ...
                if (estado == "Realizado") {
                    ...
                    attachPolygonInfoWindow(cityCircle, '<strong>' + nombre + '</strong><br/>Hora Llegada<br/>' + llegada);
                }
                else {
                ...
                if (estado == "Realizado") {
                    icono = "./images/casa_ok.png";
                    attachPolygonInfoWindow(routePath, '<strong>' + nombre + '</strong><br/>Hora Llegada<br/>' + llegada);
                }
                else {
                    icono = "./images/casa.png";
    ...
    if (is_poligono == "0") {
        ...
        if (estado == "Realizado") {
            icono = "./images/casa_ok.png";
            attachPolygonInfoWindow(cityCircle, '<strong>' + cadena + '</strong><br/>Hora Llegada<br/>' + llegada);
        }
        else {
            icono = "./images/casa.png";
        ...
        if (estado == "Realizado") {
            icono = "./images/casa_ok.png";
            attachPolygonInfoWindow(routePath, '<strong>' + cadena + '</strong><br/>Hora Llegada<br/>' + llegada);
        }
        else {
            icono = "./images/casa.png";
...
function mostrar_atributos() {
    ...
    lista_atr = document.getElementById('ListAtributos')
    var cant = lista_atr.options.length;

    alert(cant);
}

function carga_punto_alertas_ac(lat, lon, movil, fi, alerta, mi) {
...
function carga_punto_reportes_vel(lat, lon, movil, fi, vel, ale) {
...
function carga_punto_reportes_pani(lat, lon, movil, fi, vel, ale) {
...
function carga_punto_reportes_custodia(lat, lon, movil, fi, ale) {
...
function carga_punto_reportes_det(lat, lon, movil, fi, ff, mi, ale) {
...
function carga_punto_reportes(lat, lon, movil, fi, ff, mi) {
...
function carga_punto_reportes_temp(lat, lon, movil, fi) {
...
function carga_punto_reportesv(lat, lon, movil, fe, ve) {
