...
public partial class App_Reporte_Trailer : System.Web.UI.Page
{
    ...
    protected void gv_listaTrailer_Sorting(object sender, GridViewSortEventArgs e)
    {
        ...
        view.Sort = string.Format("{0} {1}", e.SortExpression, direccion);
        this.ViewState["sortExpresion"] = string.Format("{0} {1}", e.SortExpression, direccion);
        this.ViewState["lista"] = view.Table;
    ...
    public void btn_buscarTrailer_Click(object sender, EventArgs e)
    {
        TrailerBC trailer = new TrailerBC();
        int site_id = Convert.ToInt32(this.ddl_site.SelectedValue);
        DataTable dt = trailer.obtener_Reporte_XParametro(this.txt_buscarNombre.Text, this.txt_buscarNro.Text, this.chk_buscarInterno.Checked, int.Parse(this.ddl_buscarTipo.SelectedValue), int.Parse(this.ddl_buscarTransportista.SelectedValue), site_id);
